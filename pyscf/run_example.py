...
def get_magnetic_dipole_ao( mol, Shalf=None ):
    ...
    with mol.with_common_orig(nuc_dipole):
        ...
        dipole_ao  = -mol.intor("int1e_cg_irxp", comp=3, hermi=2) # pyscf uses an adtdiional hermi=2 (anti-hermitian)
    if ( Shalf is not None ):
        dipole_ao = np.einsum( 'ap,xpq,bq->xab', Shalf, dipole_ao, Shalf )
    return dipole_ao
...
def main():
 ...
 Cl                 4.94499941   -1.41543325    0.81033628
    ...
    mag_dip    = 2 * np.einsum( 'Jij,Dij->JD', X-Y, mag_dip_MO[:,occ_inds,vir_inds] ) # Dipole matrix in ES, # X-Y since observables is anti-Hermitian
    ...
    for state in range( X.shape[0] ):
        density  = np.einsum( 'xyza,ab,xyzb->xyz', AO_xyz, X_AO[state] + Y_AO[state],  AO_xyz ) # Take diagonal elements of the real-space
        ...
        myCUBE.write( field=density, fname="TD_0_%d.cube" % (state+1) )
        
        density  = np.einsum( 'ab,Dbc->Dac', X_AO[state] + Y_AO[state], el_dip_ao )
        density  = np.einsum( 'xyza,Dab,xyzb->Dxyz', AO_xyz, density,  AO_xyz ) # Take diagonal elements of the real-space
        myCUBE = cubegen.Cube( mol, nx=nx, ny=ny, nz=nz )
        #myCUBE = cubegen.Cube( mol, nx=nx, ny=ny, nz=nz, margin=0.0, origin=ORIGIN, extent=[MIN_X,MAX_X,MIN_Y,MAX_Y,MIN_Z,MAX_Z] )
        myCUBE.write( field=density[0], fname="EL_DIP_DENSITY_0_%d_X.cube" % (state+1) )
        myCUBE.write( field=density[1], fname="EL_DIP_DENSITY_0_%d_Y.cube" % (state+1) )
        myCUBE.write( field=density[2], fname="EL_DIP_DENSITY_0_%d_Z.cube" % (state+1) )

        density = np.einsum( 'ab,Dbc->Dac', X_AO[state] - Y_AO[state], mag_dip_ao )
        density = np.einsum( 'xyza,Dab,xyzb->Dxyz', AO_xyz, density, AO_xyz ) # Take diagonal elements of the real-space
        myCUBE = cubegen.Cube( mol, nx=nx, ny=ny, nz=nz )
        #myCUBE = cubegen.Cube( mol, nx=nx, ny=ny, nz=nz, margin=0.0, origin=ORIGIN, extent=[MIN_X,MAX_X,MIN_Y,MAX_Y,MIN_Z,MAX_Z] )
        myCUBE.write( field=density[0], fname="MAG_DIP_DENSITY_0_%d_X.cube" % (state+1) )
        myCUBE.write( field=density[1], fname="MAG_DIP_DENSITY_0_%d_Y.cube" % (state+1) )
        myCUBE.write( field=density[2], fname="MAG_DIP_DENSITY_0_%d_Z.cube" % (state+1) )
